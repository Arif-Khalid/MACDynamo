...
)  # path where login credentials and server details
...
def get_5m_candles(currency, lookback=0, bars=250):
...
def place_market_order(symbol, vol, buy_sell, position_id = None):
...
def trade_signal(MERGED_DF, l_s):
    ...
    if l_s == "":
        ...
        if (
            ...
            and df["macd"].tolist()[-2] > df["macd_sig"].tolist()[-2]
        ):
            signal = "Close"
...
def closeProfit(open_pos_cur):
    tot_profit = open_pos_cur.profit.sum()
    return tot_profit

def main(profits, closeAll = False):
    try:
        currentProfit = 0
        ...
        for currency in pairs:
            ...
            signal = ''
            if(closeAll):
                if(len(open_pos) > 0 and len(open_pos_cur) > 0):
                    signal = "Close"
            else:
            ...
            if signal == "Buy" or signal == "Sell":
                ...
                print(open_pos_cur.ticket)
                tot_pos = (open_pos_cur.type * open_pos_cur.volume).sum()
                currentProfit += closeProfit(open_pos_cur)
                if tot_pos > 0:
                    res = place_market_order(currency, tot_pos, "Sell", int(open_pos_cur.iloc[0].ticket))
                    print(res)
                elif tot_pos < 0:
                    res = place_market_order(currency, abs(tot_pos), "Buy", int(open_pos_cur.iloc[0].ticket))
                    print(res)
                print("All positions closed for", currency)
            elif signal == "Close_Buy":
                tot_pos = (open_pos_cur.type * open_pos_cur.volume).sum()
                currentProfit += closeProfit(open_pos_cur)
                place_market_order(currency, abs(tot_pos), "Buy", int(open_pos_cur.iloc[0].ticket))
                ...
                tot_pos = (open_pos_cur.type * open_pos_cur.volume).sum()
                currentProfit += closeProfit(open_pos_cur)
                place_market_order(currency, tot_pos, "Sell", int(open_pos_cur.iloc[0].ticket))
...
initialInvestment = 0
for currency in pairs:
    df = get_5m_candles(currency)
    print(df.Open[-1])
    initialInvestment += df.Open[-1] * 50
print("Initial Total Investment: ", initialInvestment)
profits = [initialInvestment]
times = []
while time.time() <= timeout:
    try:
        ...
        )  # 5 minute interval between each new execution
...
# %%
print(profits)
profitsSum = pd.DataFrame(profits).cumsum()
print(profitsSum)
returns = profitsSum.pct_change()
print(returns)
cum_returns = (1 + returns).cumprod()
timesDf = pd.DataFrame(times)
print(profitsSum.shift(-1).iloc[:-1])
print(returns.shift(-1).iloc[:-1])
print(timesDf)
returnsDf = pd.concat([timesDf,profitsSum.shift(-1).iloc[:-1],returns.shift(-1).iloc[:-1],cum_returns.shift(-1).iloc[:-1]], axis=1)
returnsDf.columns=['time', 'cum_profit', 'ret', 'cum_ret']
returnsDf.set_index('time')
print(returnsDf)
